---  
layout: post  
title: "[GitHub] 깃은 어떻게 파일을 관리하나요?"
categories: others
tags: others project github file UNIX
comments: true
---
...
👩‍💻 출처: [GitHub Book](https://book.git-scm.com/book/en/v2){: target="_blank"}
...
- 파일의 크기
등 여러가지 정보와 함께 파일에 마지막으로 접근한 시각, 파일을 마지막으로 수정한 시각 등 시간 정보도 함께 들어있다. 더 자세한 내용은 [이곳](https://www.thegeekstuff.com/2009/07/unix-stat-command-how-to-identify-file-attributes/){: target="_blank"}에서 확인할 수 있다.
...
위와 같이 마지막에 두 줄을 추가하고 푸시를 해보았다. 그랬더니...

- 추가하기 전

![testing](https://broccolism.github.io/assets/img/others/2020-05-21-9.JPG){: width = "90%" } 

- 추가한 후

![testing](https://broccolism.github.io/assets/img/others/2020-05-21-10.JPG){: width = "90%" } 
이상한 일이 일어났다. 같은 파일에 대한 key가 아예 바뀐 것이다! 완전히 다른 blob 객체가 되었다.

*"깃은 객체를 압축할 때 비슷한 이름과 크기를 가진 파일을 찾고, 한 버전에서 다음 버전으로의 차이만 저장합니다."*

그래서 최종적으로 저장되는 결과물은 1. 가장 최근에 저장한 버전의 파일과 그 이전 버전 파일의 **차이** 2. 가장 최근에 저장한 버전의 파일 이렇게 두가지가 된다. 파일의 원본 버전을 온전히 저장하지 않고 이런 방식을 사용하는 이유는 가장 최선 버전의 파일에 더 빨리 접근해야 하기 때문이라고 한다.

사실 이 부분이 원래 해결하고자 했던 의문점에 대한 핵심 답변인데, 더 자세한 내용은 적혀있지 않아 아쉬웠다. 대신 모든 파일을 지우고 pull 명령어를 실행했을 때 아무 일도 일어나지 않은 원인은 찾을 수 있었다.

# Git Merge가 일어나는 방법
무려 **Diff3** 알고리즘을 사용한다. 해당 알고리즘에 대한 자세한 내용은 [이 논문](https://www.cis.upenn.edu/~bcpierce/papers/diff3-short.pdf){: target="_blank"}에서 확인할 수 있다.

🔥 "diff3는 수정된 두 버전의 변경 사항을 공통 이전 버전으로 통합할 수 있습니다. 이를 통해 사용자는 두 개의 최신 파일로 표시되는 변경 사항을 하나로 병할할 수 있습니다."

`git pull` 명령어를 수행하면 깃에서 내부적으로 `git fetch` 후 `git merge`를 실행시킨다. 이 때 `git merge` 명령어를 입력하면 **두 사용자가 편집한 파일의 변경 사항을 보고** 하나의 파일로 합쳐주는 것이다.

지금까지는 `git pull origin A`나 `git merge A`를 하면 항상 A라는 브랜치의 작업 내용을 현재 브랜치로 덮어씌우는 줄로만 알고 있었다. 그래서 완성본을 싹 다 날려버린 것이다. 두 브랜치의 작업물을 merge할 때 한 쪽 브랜치에서 모든 파일을 지워버리면 지운 파일의 변경 사항을 알 수 없기 때문에 `git pull`이 제대로 되지 않은게 아니었을까...

# 💭
GitHub 사용 빈도가 늘면서 그 내부 작동 원리가 항상 궁금했는데 이번 기회에 아주 조금이나마 알게 된 것 같다. 특히 *diff3*이라는 알고리즘이 논문으로 나와있는걸 보면서 역시 깃이 그냥 대충 만든게 아니구나, 하는 생각을 새삼 하게 되었다. 지금 쌓여있는 과제를 모두 다 끝내고, 시험도 다 끝나서 방학이 되면 diff3 알고리즘 논문을 한 번 읽어봐야겠다. 두근두근!